...
| `execute` | User confirmed — broadcast Uni V2 swap (reuses fresh quote payload) |
...
4. `quote` then show `preview` (you pay / you receive, rate, gas, optional `usd`, warnings).
5. After user approval, `execute` with the same intent and `confirmed: true`.
